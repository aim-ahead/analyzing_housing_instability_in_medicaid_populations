...
write.table(t1_withOverall, file= "outputs/t1_withOverall_from2016to2021.txt", col.names = TRUE, row.names = TRUE, sep = "\t", quote = FALSE)
```
...
doc <- doc %>%
  body_add_par("Table 1. Housing instability before and after pandemic (with overall") %>%
  body_add_table(  t1_withOverall )

# Save the document
print(doc, target = "outputs/t1_withOverall_from2016to2021.docx")
```
...
write.table(table1_output, file= "outputs/t1_withPvalues_from2016to2021.txt", col.names = TRUE, row.names = TRUE, sep = "\t", quote = FALSE)
```
...
# Add content to the document
doc <- doc %>%
  body_add_par("Table 1. Housing instability before and after pandemic (with p-value)") %>%
  body_add_table(  table1_output )

# Save the document
print(doc, target = "outputs/t1_withPvalues_from2016to2021.docx")
```
...
# Add content to the document
doc <- doc %>%
  body_add_par("Housing insecurity by year") %>%
  body_add_table(  byYearTable )

# Save the document
print(doc, target = "outputs/byYearTable.docx")
```
