...
```{r}
encounters <- read.delim("reviewSQLoutputs/f1_first_encounter_date.txt", header = TRUE, sep = ",")
...
table1(~ age_group + sex + hispanic + race + eth_race + language + CURRENT_FPL_CD + HOMELESS_CD + maritalstatus + 
...
write.table(table1_output, file= "outputs/table1.txt", col.names = TRUE, row.names = TRUE, sep = "\t", quote = FALSE)
```
