...
sdh_encounters <- read.delim("reviewSQLoutputs/f1_all_payors_housingInstability_encounter_screened_rownum44660.txt", header = TRUE, sep = ",")
sdh_demographics <- read.delim("reviewSQLoutputs/f1_all_payors_housingInstability_demographics_screened_rownum31288.txt", header = TRUE, sep = ",")
```
...
```
# Add mapping from zip code to state
```{r}
load("outputs/zipCodeWithStateMap.RData")
all_patients_info <- all_patients_info %>%
  dplyr::left_join( xx, by=c("zip_cd"= "ZIP_CD"))
```
...
save( all_patients_info_categorized, file = "/home/rstudio-user/studies/Research-Fellowship-jovita-murillo/all_patients_info_categorized_screened.RData")
```
...
write.table(allPopTable, file= "outputs/allPopTableByPayorType_screened.txt", col.names = TRUE, row.names = TRUE, sep = "\t", quote = FALSE)
...
doc <- doc %>%
  officer::body_add_par("Table X. Patients with housing insecurity: description by payor type") %>%
  officer::body_add_table(  as.data.frame( allPopTable ) )

# Save the document
print(doc, target = "outputs/allPopTableByPayorType_screened.docx")
```
